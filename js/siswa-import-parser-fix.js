...
(function () {
  ...
  async function handleFile(input){
    const modal=document.getElementById("gtrImportSiswaModal");if(!modal||!input.files?.[0])return;const preview=modal.querySelector("#gtrImportPreview"),button=modal.querySelector("#gtrImportRun"),text=modal.querySelector("#gtrImportReadyText");
    try{preview.innerHTML=`<div class="gtr-import-loading">Membaca data siswa...</div>`;text.textContent="Membaca file...";button.disabled=true;const rows=objects(await input.files[0].text());const {data:existing,error}=await supabase.from("siswa").select("nis");if(error)throw error;const set=new Set((existing||[]).map(x=>String(x.nis||"").trim().toLowerCase()).filter(Boolean));candidates=makeCandidates(rows,set);const result=render(preview,candidates);button.disabled=result.valid.length===0;text.textContent=result.valid.length?`${result.valid.length} siswa siap diimport${result.invalid.length?` · ${result.invalid.length} perlu diperiksa`:""}.`:"Belum ada data yang siap diimport.";}catch(error){console.error(error);candidates=[];button.disabled=true;text.textContent="File belum siap diimport.";preview.innerHTML=`<div class="gtr-import-error">⚠ ${esc(error?.message||"File tidak dapat dibaca.")}</div>`;}
  }
